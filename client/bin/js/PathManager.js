...
var PathManager = /** @class */ (function () {
    ...
    PathManager.prototype.__createPrefabs = function () {
        var t = new Laya.Sprite3D;
        SceneManager.Instance.addToScene(t);
        var e = GameData.Instance.dist;
        var i = GameData.Instance.qOffset;
        var n = GameData.Instance.sceneIndex;
        var r = GameData.Instance.fogStart;
        var a = GameData.Instance.fogStartD;
        var s = GameData.Instance.fogRange;
        var o = GameData.Instance.fogRangeD;
        var h = GameData.Instance.fogColor;
        ...
        for (c = 0; c < this.__boxCount; c++) {
            ...
            y.size = new Laya.Vector3(1, 1, 1);
            y.enable = false;
            this.__boxObstacles.push(x);
        }
        var d = Laya.Mesh.load("res/mesh/speedUp.lm");
        ...
        for (var f = new Laya.Vector3(1, 1.2, .3), p = new Laya.Vector3(-90, 0, 0), c = 0; c < this.__speedUpCount; c++) {
            ...
            (y = x.addComponent(Laya.BoxCollider)).size = new Laya.Vector3(1, 1, 1.5);
            y.enable = false;
        ...
        for (var m = new Laya.Vector3(1, 1, Math.SQRT2), g = new Laya.Vector3(45, 0, 0), c = 0; c < this.__jumpCount; c++) {
            var v = new Laya.Sprite3D("jump");
            var x = new Laya.MeshSprite3D(l);
            v.addChild(x);
            x.meshRender.sharedMaterial = this.__jumpMat;
            x.transform.localScale = m;
            x.transform.localRotationEuler = g;
            v.active = false;
            t.addChild(v);
            v.layer = Laya.Layer.getLayerByNumber(2); //jump台的物理层是1
            var y = v.addComponent(Laya.BoxCollider);
            y.size = new Laya.Vector3(1, 1, 1);
            y.enable = false;
            this.__jumpObstacles.push(v);
        }
        var E = Laya.Layer.getLayerByNumber(1); //jump,障碍物，加速器
        var T = Laya.Layer.getLayerByNumber(0); //小球
        var M = Laya.Layer.getLayerByNumber(2);
    ...
    PathManager.prototype.__createObstacleControls = function () {
    ...
    PathManager.prototype.__updateEnviroment = function () {
        this.__updateRoads();
        this.__updateBuildings();
    };
    PathManager.prototype.__updateRoads = function () {
    ...
    PathManager.prototype.__updateObstacles = function (t) {
        for (n = 0; n < this.__fixedObstacleControlCount; n++) {
            var e = this.__fixedObstacleControlPool[n];
            if (e.active)
                e.update(t);
        }
        for (n = 0; n < this.__moveObstacleControlCount; n++) {
            var i = this.__moveObstacleControlPool[n];
            if (i.active)
                i.update(t);
        }
        for (var n = 0; n < this.__chainObstacleControlCount; n++) {
            var r = this.__chainObstacleControlPool[n];
            if (r.active)
                r.update(t);
        }
    };
    PathManager.prototype.__createObstacle = function () {
        if (!(this.__roadSpanZ > this.__initCreateObstacleDis))
            return this.__createSpeedUpDis--,
    ...
    PathManager.prototype.__setHasOrNoObstacles = function () {
        this.__createSimpleCounter < 0 ? this.__setObstacles() : this.__setSimpleObstacles();
    };
    PathManager.prototype.__setObstacles = function (t, e, i, n) {
        var r = Math.random();
        var a = 0;
        var s = 0;
        var o = -1;
        ...
        switch (h) {
            ...
            case 8:
                ...
                break;
            case 9:
                var f = Math.random() > .5 ? MoveObstacleType.SINGLELEFTTORIGHT : MoveObstacleType.DOUBLELEFTANDRIGHT;
                var p = Math.random() > .5;
                void 0 !== n && null !== n && (f = n);
                this.__setMoveObstacles({
                    type: f,
            ...
            case 16:
                this.__createSimpleCounter = 0;
                var m = Math.floor(7 * Math.random()) + 2;
                Math.random();
    ...
    PathManager.prototype.__setSimpleObstacles = function () {
        var t = this.__createSimpleTypes[this.__createSimpleCounter];
        this.__createSimpleCounter++;
        this.__setObstacles(t["type"], void 0 !== t["otherType"] ? t["otherType"] : null, void 0 !== t["lorr"] ? t["lorr"] : null, void 0 !== t["moveType"] ? t["moveType"] : null);
        if (this.__createSimpleCounter >= this.__createSimpleTypes.length) {
            this.__createSimpleCounter = -1;
            this.__createSimpleTypes = null;
        }
    };
    PathManager.prototype.__setFixedObstacles = function (t) {
        var e = {
            type: -1,
            otherType: -1,
    ...
    PathManager.prototype.__setMoveObstacles = function (t) {
        var e = {
            type: -1,
            lorr: false,
            once: false,
    ...
    //luo luo luo 
    PathManager.prototype.__setChainObstacles = function (t) {
        var e = {
            type: -1,
            spanZ: this.__roadSpanZ,
            boxes: null
    ...
    PathManager.prototype.__getBoxes = function (t) {
        for (var e = [], i = 0; i < t; i++) {
            var n = this.__boxObstacles[this.__boxIndex];
            e.push(n);
            this.__boxIndex = this.__boxIndex >= this.__boxCount - 1 ? 0 : this.__boxIndex + 1;
        }
        return e;
    };
    PathManager.prototype.__getJumps = function (t) {
        for (var e = [], i = 0; i < t; i++) {
            var n = this.__jumpObstacles[this.__jumpIndex];
            e.push(n);
            this.__jumpIndex = this.__jumpIndex >= this.__jumpCount - 1 ? 0 : this.__jumpIndex + 1;
        }
        return e;
    };
    PathManager.prototype.__getSpeedUps = function (t) {
        for (var e = [], i = 0; i < t; i++) {
            var n = this.__speedUpObstacles[this.__speedUpIndex];
            e.push(n);
            this.__speedUpIndex = this.__speedUpIndex >= this.__speedUpCount - 1 ? 0 : this.__speedUpIndex + 1;
        }
        return e;
    };
    PathManager.prototype.__updateBuildings = function () {
        var t = PlayerControl.Instance.curPos.z;
        if (t - this.__buildingLSpanZ < this.__spanZDis) {
            var e = this.__buildingsPrefab[this.__buildingsIndex];
            var i = Math.random() * (this.__buildingScaleYMax - this.__buildingScaleYMin) + this.__buildingScaleYMin;
            e.active = true;
    ...
    PathManager.prototype.gameUpdate = function (t) {
    ...
    PathManager.prototype.gameReset = function () {
        this.levelChange();
        this.__resetAllObjs();
    };
    PathManager.prototype.__disableObj = function (t) {
        t.active = false;
        var e = t.getComponentByType(Laya.BoxCollider);
        if (e) {
            e.enable = false;
        }
    };
    PathManager.prototype.__resetAllObjs = function () {
        this.__roadsIndex = 0;
        this.__roadSpanZ = 6;
        this.__roadSteps = 0;
        ...
        this.__buildingLSpanZ = 5;
        this.__buildingRSpanZ = 2;
        for (t = 0; t < this.__buildingCount; t++)
            this.__disableObj(this.__buildingsPrefab[t]);
        this.__boxIndex = 0;
        for (t = 0; t < this.__boxCount; t++)
            this.__disableObj(this.__boxObstacles[t]);
        this.__jumpIndex = 0;
        for (t = 0; t < this.__jumpCount; t++)
            this.__disableObj(this.__jumpObstacles[t]);
        this.__speedUpIndex = 0;
        for (t = 0; t < this.__speedUpCount; t++)
            this.__disableObj(this.__speedUpObstacles[t]);
        this.__fixedObstacleControlIndex = 0;
        for (t = 0; t < this.__fixedObstacleControlCount; t++)
            this.__fixedObstacleControlPool[t].hide();
        this.__moveObstacleControlIndex = 0;
        for (t = 0; t < this.__moveObstacleControlCount; t++)
            this.__moveObstacleControlPool[t].hide();
        this.__chainObstacleControlIndex = 0;
        for (var t = 0; t < this.__chainObstacleControlCount; t++)
            this.__chainObstacleControlPool[t].hide();
        this.__createSpeedUpDis = 0;
        this.__createSimpleCounter = -1;
        this.__createSimpleTypes = null;
    };
    PathManager.prototype.__qOffsetChange = function () {
        //这里会触发一个场景的变换！
        if (GameManager.Instance.gameState === GameState.PLAYING) {
            var t = GameData.Instance.qOffset;
            var e = Math.floor(Math.random() * this.__qOffsetCountX);
            // debugger
            if (this.__qOffsetXIndex === e) {
                e > 0 ? e -= 1 : e += 1;
            }
            this.__qOffsetXIndex = e;
            var i = Math.floor(Math.random() * this.__qOffsetCountY);
            if (this.__qOffsetYIndex === i) {
                i > 0 ? i -= 1 : i += 1;
            }
            this.__qOffsetYIndex = i;
            var n = Math.abs(this.__qOffsetXS[this.__qOffsetXIndex] - t.x);
            var r = Math.abs(this.__qOffsetYS[this.__qOffsetYIndex] - t.y);
    ...
    PathManager.prototype.sceneChange = function (t) {
        var e = GameData.Instance.sceneIndex;
        var i = GameData.Instance.fogColor;
